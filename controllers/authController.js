...
const handleWelcomeMessage = async (req, res) => {
  try {
    return res
      .status(200)
      .json({ message: "Welcome to Fintech Digital Wallet System " });
  } catch (error) {
...
const handleUserRegistration = async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res
        .status(404)
        .json({ message: "User account already exist. login" });
    }
    ...
    return res
      .status(201)
      .json({ message: "Registration successfull", result, wallet });
  } catch (error) {
...
const handleUserLogin = async (req, res) => {
  try {
    ...
    return res.status(200).json(
      { message: "Login successfull", user, accessToken },
